...
ACCOUNT:
  An account could be a regular Silo/MinIO user, STS or LDAP user.

FLAGS:
  {{range .VisibleFlags}}{{.}}
  {{end}}
EXAMPLES:
  1. Add a new service account for user 'foobar' to Silo/MinIO server with a name and description.
     {{.Prompt}} {{.HelpName}} mysilo foobar --name uploaderKey --description "foobar uploader scripts"

  2. Add a new service account to Silo/MinIO server with specified access key and secret key for user 'foobar'.
     {{.Prompt}} {{.HelpName}} mysilo foobar --access-key "myaccesskey" --secret-key "mysecretkey"

  3. Add a new service account to Silo/MinIO server with specified access key and random secret key for user 'foobar'.
     {{.Prompt}} {{.HelpName}} mysilo foobar --access-key "myaccesskey"

  4. Add a new service account to Silo/MinIO server with specified secret key and random access key for user 'foobar'.
     {{.Prompt}} {{.HelpName}} mysilo foobar --secret-key "mysecretkey"

  5. Add a new service account to Silo/MinIO server with specified expiry date in the future for user 'foobar'.
     {{.Prompt}} {{.HelpName}} mysilo foobar --expiry 2023-06-24T10:00:00-07:00
`,
}
