...
FLAGS:
  ...
  {{end}}
EXAMPLES:
  1. Add replication configuration rule on bucket "sourcebucket" for alias "sourcesilo" with alias "targetsilo" to replicate all operations in an active-active replication setup.
     {{.Prompt}} {{.HelpName}} sourcesilo/sourcebucket --remote-bucket targetsilo/targetbucket \
         --priority 1 

  2. Add replication configuration rule on bucket "mybucket" for alias "mysilo" to replicate all operations in an active-active replication setup.
     {{.Prompt}} {{.HelpName}} mysilo/mybucket --remote-bucket https://foobar:foo12345@silo.siteb.example.com/targetbucket \
         --priority 1 

  3. Add replication configuration rule on bucket "mybucket" for alias "mysilo" to replicate all objects with tags
  ...
  4. Disable a replication configuration rule on bucket "mybucket" for alias "mysilo".
     {{.Prompt}} {{.HelpName}} mysilo/mybucket --remote-bucket https://foobar:foo12345@silo.siteb.example.com/targetbucket  \
  ...
  5. Add replication configuration rule with existing object replication, delete marker replication and versioned deletes
     enabled on bucket "mybucket" for alias "mysilo".
     {{.Prompt}} {{.HelpName}} mysilo/mybucket --remote-bucket https://foobar:foo12345@silo.siteb.example.com/targetbucket  \
