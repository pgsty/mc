...
EXAMPLES:
  1. Add a lifecycle rule with a transition and a noncurrent version transition action for objects with prefix doc/ whose size is greater than 1MiB in mybucket.
     ...
     {{.Prompt}} mc ilm tier add minio mysilo SILOTIER-1 --endpoint https://warm-silo-1.com \
         --access-key ACCESSKEY --secret-key SECRETKEY --bucket bucket1 --prefix prefix1

     {{.Prompt}} {{.HelpName}} --prefix "doc/" --size-gt 1MiB --transition-days "90" --transition-tier "SILOTIER-1" \
         --noncurrent-transition-days "45" --noncurrent-transition-tier "SILOTIER-1" \
         mysilo/mybucket/

  2. Add a lifecycle rule with an expiration action for all objects in mybucket.
     {{.Prompt}} {{.HelpName}} --expire-days "200" mysilo/mybucket
...
var ilmAddFlags = []cli.Flag{
	...
	cli.StringFlag{
		Name:   "storage-class",
		Usage:  "storage class for current version to transition into. Silo/MinIO supports tiers configured via `mc-admin-tier-add`.",
		Hidden: true,
	},
	cli.StringFlag{
		Name:   "tier",
